...
*/
void con_list_init(con_list *list, size_t typSze, size_t cpcty) {
  list->head = calloc(cpcty, typSze * 2);
  list->cpcty = cpcty;
  list->typSze = typSze * 2;
  list->size = 0;
}
...
*/
void con_list_add(con_list *list, void *ele) {
...
*/
void con_list_dltIndx(con_list *list, size_t i) {
  free(list->head[i]);
  for (size_t j = i; j < list->size-1; ++j) list->head[j] = list->head[j + 1];
...
*/
void *con_list_get(con_list *list, size_t i) {
  return list->head[i];
}
...
*/
size_t con_list_size(con_list *list) {
  return list->size;
}
...
*/
void con_list_clr(con_list *list) {
...
*/
void con_list_setAt(con_list *list, size_t i, void *ele) {
  list->head[i] = ele;
}
...
*/
_Bool con_list_cntns(con_list *list, void *ele, int(compareTo)(void *, void *)) {
  for (int i = 0; i < list->size; ++i) {
...
*/
_Bool con_list_isEmpty(con_list *list) {
  return list->size == 0;
}
...
*/
void con_list_grow(con_list *list) {
  list->cpcty = 2 * list->cpcty;
  list->head = realloc(list->head, list->cpcty * list->typSze);
}
...
*/
void con_list_free(con_list *list) {
  free(list->head);
}
